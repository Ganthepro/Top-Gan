...
filename = "HT19-C00367-12S190011-12  Color.tif" #HT19-C00514-NHU036814-2SAM_CARVE.tif #H20-C00175-11S190500-04-sample.tif
...
for i in vec:
    if (i[sy] > i[ey] or i[sy] < i[ey]) and i[sx] == i[ex]:
        ...
        p2 = [(i[ex] * scale / 1) * 0.35 + 20, (i[ey] * scale / 1) * 0.35 + width_y + 20, z]#-+-
        p3 = [(i[sx] * scale / 1) * 0.35 + width_x + 20, (i[sy] * scale / 1) * 0.35 + 20, z]#+--
        p4 = [(i[ex] * scale / 1) * 0.35 + width_x + 20, (i[ey] * scale / 1) * 0.35 + width_y + 20, z]#++-
        p5 = [(i[sx] * scale / 1) * 0.35 + 20, (i[sy] * scale / 1) * 0.35 + 20, z + height]#--+
        p6 = [(i[ex] * scale / 1) * 0.35 + 20, (i[ey] * scale / 1) * 0.35 + width_y + 20, z + height]#-++
        p7 = [(i[sx] * scale / 1) * 0.35 + width_x + 20, (i[sy] * scale / 1) * 0.35 + 20, z + height]#+-+
        p8 = [(i[ex] * scale / 1) * 0.35 + width_x + 20, (i[ey] * scale / 1) * 0.35 + width_y + 20, z + height]#+++
        vertices = np.array([p1, p2, p3, p4, p5, p6, p7, p8])
        face1_1 = np.array([p4, p7, p8]);
        face1_2 = np.array([p7, p3, p4])
        face2_1 = np.array([p8, p4, p2]);
        face2_2 = np.array([p2, p6, p8])
        face3_1 = np.array([p2, p6, p5]);
        face3_2 = np.array([p2, p1, p5])
        face4_1 = np.array([p5, p1, p3]);
        face4_2 = np.array([p3, p7, p5])
        face5_1 = np.array([p7, p8, p6]);
        face5_2 = np.array([p6, p5, p7])
        face6_1 = np.array([p1, p3, p4]);
        face6_2 = np.array([p4, p2, p1])
        ...
        p1 = [i[sx] * scale / 1 * 0.35 + 20, i[sy] * scale / 1 * 0.35 + 20, z]#---
        p2 = [i[sx] * scale / 1 * 0.35 + 20, (i[sy] * scale / 1) * 0.35 + width_y + 20, z]#-+-
        p4 = [i[ex] * scale / 1 * 0.35 + 20, (i[ey] * scale / 1) * 0.35 + width_y + 20, z]#++-
        p3 = [i[ex] * scale / 1 * 0.35 + 20, i[ey] * scale / 1 * 0.35 + 20, z]#+--
        p5 = [i[sx] * scale / 1 * 0.35 + 20, i[sy] * scale / 1 * 0.35 + 20, z + height]#--+
        p6 = [i[sx] * scale / 1 * 0.35 + 20, (i[sy] * scale / 1 * 0.35) + width_y + 20, z + height]#-++
        p8 = [i[ex] * scale / 1 * 0.35 + 20, (i[ey] * scale / 1 * 0.35) + width_y + 20, z + height]#+++
        p7 = [i[ex] * scale / 1 * 0.35 + 20, i[ey] * scale / 1 * 0.35 + 20, z + height]#+-+
        vertices = np.array([p1, p2, p3, p4, p5, p6, p7, p8])
        face1_1 = np.array([p4, p7, p8]);
        face1_2 = np.array([p7, p3, p4])
        face2_1 = np.array([p8, p4, p2]);
...
#for i in vec:
p1 = [19, 20, 0]#---
p2 = [19, (n_hy * 0.35) + 21, 0]#-+-
p3 = [(n_wx * 0.35) + 20, 20, 0]#+--
p4 = [(n_wx * 0.35) + 20, (n_hy * 0.35) + 21, 0]#++-
p5 = [19, 20, z]#--+
p6 = [19, (n_hy * 0.35) + 21, z]#-++
p7 = [(n_wx * 0.35) + 20, 20, z]#+-+
p8 = [(n_wx * 0.35) + 20, (n_hy * 0.35) + 21, z]#+++
face1_1 = np.array([p4, p7, p8]);face1_2 = np.array([p7, p3, p4])
face2_1 = np.array([p8, p4, p2]);face2_2 = np.array([p2, p6, p8])
face3_1 = np.array([p2, p6, p5]);face3_2 = np.array([p2, p1, p5])
face4_1 = np.array([p5, p1, p3]);face4_2 = np.array([p3, p7, p5])
face5_1 = np.array([p7, p8, p6]);face5_2 = np.array([p6, p5, p7])
face6_1 = np.array([p1, p3, p4]);face6_2 = np.array([p4, p2, p1])
